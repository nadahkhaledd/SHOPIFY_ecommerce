            ...
            <div class="testbox">
                <form:form modelAttribute="product" method="POST" >
                    <div class="banner" style="background-color:maroon">
                        <h1>Update product <i>${product.name}</i></h1>
                    </div>
                    <div class="columns">
                        <div class="item">
                            <label for="name">Product name<span>*</span></label>
                            <form:input path="name" id="name" value="${name}" type="text" name="name" required="true"/>
                            <form:errors path="name" cssClass="error"/>
                        </div>

                        <div class="item">
                            <label for="imagePath">Image path<span>*</span></label>
                            <form:input path="imagePath" value="${imagePath}" id="imagePath" type="text" name="imagePath" required="true"/>
                            <form:errors path="imagePath" cssClass="error"/>
                        </div>

                        <div class="item">
                            <label for="price">Price<span>*</span></label>
                            <form:input path="price" value="${price}" id="price" type="number" min="1" step="1" name="price" required="true"/>
                            <form:errors path="price" cssClass="error"/>
                        </div>

                        <div class="item">
                            <label for="category">Category<span>*</span></label>
                            <form:select path="category" value="${category.name}" multiple="false" id="category" name="category" required="true" items="${categories}" />
                            <form:errors path="category" cssClass="error"/>
                        </div>

                        <div class="item">
                            <label for="availableQuantity">availableQuantity<span>*</span></label>
                            <form:input path="availableQuantity" value="${availableQuantity}" id="availableQuantity" required="true" type="number" min="0" step="1" name="availableQuantity"/>
