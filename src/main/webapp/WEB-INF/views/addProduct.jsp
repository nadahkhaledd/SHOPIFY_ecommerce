              ...
              <form:form modelAttribute="product" method="POST" >
                ...
                <div class="columns">
                  <div class="item">
                    ...
                    <form:input path="name" id="name" type="text" required="required" name="name"/>
                    <form:errors path="name" cssClass="error"/>
                  </div>
                  <div class="item">
                    <label for="imagePath">Image path<span>*</span></label>
                    <form:input path="imagePath" id="imagePath" required="required" type="text" name="imagePath"/>
                    <form:errors path="imagePath" cssClass="error"/>
                  </div>

                  <div class="item">
                    <label for="price">Price<span>*</span></label>
                    <form:input path="price" id="price" type="number"  required="required" min="1" step="1" name="price"/>
                    <form:errors path="price" cssClass="error"/>
                  </div>

                  <div class="item">
                    <label for="category">Category<span>*</span></label>
                    <form:select path="category" multiple="false" id="category" required="required" name="category" items="${categories}" />
                    <form:errors path="category" cssClass="error"/>
                  </div>

                  <div class="item">
                    <label for="availableQuantity">availableQuantity<span>*</span></label>
                    <form:input path="availableQuantity" id="availableQuantity" required="required" type="number" min="0" step="1" name="availableQuantity"/>
